...
- When a repository defines release targets in `Makefile`, prefer those targets over ad-hoc release commands. Use the repository's release-check target to run validation before publishing, use its release target to publish, and use its bump target when preparing a version-bump PR.
